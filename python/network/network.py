...
class Network(list):
    ...
    def __init__(self, init_nodes=None):
        ...
        if init_nodes:
            ...
            base_station.pos_d = cf.BS_POS_D
    ...
    def simulate(self):
        ...
        for round_nb in range(0, cf.MAX_ROUNDS):
            ...
            tracer['dead_nodes'][2].append(cf.NB_NODES-nb_alive_nodes)
            if cf.TRACE_ENERGY:
                tracer['energy_left'][2].append(self.get_remaining_energy())
                tracer['energy_consumed'][2].append(
                    cf.INITIAL_ENERGY * cf.NB_NODES - self.get_remaining_energy())
    ...
    def get_active_nodes(self):
        ...
        awake."""
        def is_active(x): return x.alive and not x.is_sleeping
